  ...
  useEffect(() => {
    if (currentModel) {
      ...
      if (modelTypeConfig && modelTypeConfig.providerOptions.length > 0) {
        // 首先尝试精确匹配 baseUrl
        const providerMatch = modelTypeConfig.providerOptions.find(p => p.baseUrl === currentModel.baseUrl);
        if (providerMatch) {
              ...
              <Flex gap={6}>
                ...
                <Box flex="1">
                  <Field.Root mb={4} invalid={!!errors.modelName}>
                    ...
                    />
                    {errors.modelName && (
                      <Field.ErrorText>{errors.modelName}</Field.ErrorText>
                    )}
                  </Field.Root>
                  ...
                  {shouldShowBaseUrlInput && (
                    <Field.Root mb={4} invalid={!!errors.baseUrl}>
                      ...
                      />
                      {errors.baseUrl && (
                        <Field.ErrorText>{errors.baseUrl}</Field.ErrorText>
                      )}
                    </Field.Root>
                  )}
