...
export function OptimizationHeader() {
  ...
  const { deleteTask, tasks, startOptimization, stopOptimization, currentTaskId } =
  ...
  return (
    ...
    >
      ...
      >
        <Box flexGrow={1}>
          ...
          <Text fontSize="sm" color="gray.500" mt={1}>
            ...
            <Text as="span" fontWeight="medium" color="gray.700">
              {currentTask.datasetName}
            </Text>{' '}
            | 模式:{' '}
            <Text as="span" fontWeight="medium" color="gray.700">
              {currentTask.testSet.mode === 'strict' ? '严格模式' : '描述性模式'}
            </Text>
          </Text>
        </Box>
        <Box textAlign={isMobile ? 'left' : 'right'} flexShrink={0}>
          <Flex alignItems="center" justifyContent={isMobile ? 'flex-start' : 'flex-end'} gap={2}>
            ...
            <Badge
              colorPalette={
                status === 'completed' ? 'blue' : status === 'in_progress' ? 'green' : 'gray'
              }
              px={2}
              py={1}
              borderRadius="md"
            ...
            <Flex alignItems="center" gap={2}>
              {status === 'in_progress' && <Spinner size="sm" color="blue.500" />}
              {status === 'in_progress' ? (
                <Button size="sm" variant="outline" onClick={handleStopOptimization}>
                  <FiStopCircle />
                  <Text>停止优化</Text>
                </Button>
              ) : status === 'paused' ? (
            ...
            <IconButton size="sm" variant="outline" onClick={onParamsOpen}>
              <MdTune />
            </IconButton>
            <IconButton size="sm" variant="outline" onClick={onSettingsOpen}>
              <FiSettings />
            </IconButton>
            <IconButton variant="outline" size="sm" colorPalette="red" onClick={onDeleteOpen}>
              <FiTrash2 />
            </IconButton>
          </Flex>
          <Flex justifyContent={isMobile ? 'flex-start' : 'flex-end'} gap={4} mt={1}>
            <Text fontSize="sm" color="gray.500">
              总迭代次数:{' '}
              <Text as="span" fontWeight="medium" color="gray.700">
                {currentTask.promptIterations.length}
              </Text>
            </Text>
            <Text fontSize="sm" color="gray.500">
              目标模型:{' '}
              <Text as="span" fontWeight="medium" color="gray.700">
                输入{currentTask.targetModelTokenUsage?.promptTokens || 0}/输出
                {currentTask.targetModelTokenUsage?.completionTokens || 0}
              </Text>
            </Text>
            <Text fontSize="sm" color="gray.500">
              优化模型:{' '}
              <Text as="span" fontWeight="medium" color="gray.700">
                输入{currentTask.optimizationModelTokenUsage?.promptTokens || 0}/输出
                {currentTask.optimizationModelTokenUsage?.completionTokens || 0}
              </Text>
            </Text>
          </Flex>
        </Box>
      ...
      {/* 使用抽离出的设置对话框组件 */}
      <TaskSettingsDialog isOpen={isSettingsOpen} onClose={onSettingsClose} taskId={taskId} />

      {/* 使用抽离出的参数设置对话框组件 */}
      <ModelParamsDialog isOpen={isParamsOpen} onClose={onParamsClose} taskId={taskId} />
