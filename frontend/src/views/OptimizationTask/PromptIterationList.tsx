...
export function PromptIterationList() {
  ...
  const currentTask = useOptimizationStore(state =>
    state.tasks.find(t => t.id === state.currentTaskId)
  );
  const { submitUserFeedback, tasks, currentTaskId, closeSummary, showSummary } =
  ...
  const getStageBadge = (item: any) => {
    ...
    const currentIteration = tasks
      .find(task => task.id === currentTaskId)
      ?.promptIterations.find(iteration => iteration.id === item.id);
    ...
    return (
      <Flex alignItems="center" gap={2} width="100%">
        ...
        {item.avgScore !== null && (stage === 'evaluated' || stage === 'summarized') && (
          <Badge fontSize="xs" color="white" bg={getScoreBadgeColor(item.avgScore)}>
