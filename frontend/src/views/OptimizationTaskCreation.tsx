...
export function OptimizationTaskCreation({ onUpload }: NoDatasetViewProps) {
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        if (!Array.isArray(jsonData)) {
          setErrors({
            testCases:
              'JSON 文件内容必须是一个测试用例数组。例如：[{"input": "...", "output": "..."}, ...]',
          });
        ...
        const newTestCases: TestCase[] = jsonData.map((item: any, index: number) => {
          if (typeof item.input === 'undefined' && typeof item.output === 'undefined') {
            throw new Error(
              `文件中第 ${index + 1} 个测试用例格式错误，"input" 和 "output" 字符串字段至少需要一个。`
            );
          }
          return {
        ...
        setErrors({
          testCases:
            err instanceof Error
              ? err.message
              : '文件格式无效或内容错误，请上传有效的 JSON 测试用例数组。',
        });
        if (e.target) e.target.value = ''; // Clear file input
        setSelectedFile(null);
      }
  ...
  return (
    <Flex flexDirection="column" alignItems="center" justifyContent="flex-start" pt={10} pb={6}>
      <Icon as={FiLayers} w={12} h={12} color="gray.400" mb={4} />
      ...
      <Box w="100%" maxW="800px" bg="white" borderRadius="lg" shadow="md" p={6}>
        <VStack align="stretch" gap={6}>
          ...
          <Field.Root invalid={!!errors.initialPrompt}>
            ...
            />
            {errors.initialPrompt && <Field.ErrorText>{errors.initialPrompt}</Field.ErrorText>}
          </Field.Root>
          ...
          <Field.Root invalid={!!errors.testCases}>
            ...
            {errors.testCases && <Field.ErrorText>{errors.testCases}</Field.ErrorText>}
          </Field.Root>
